...
/* ========================================
 * 8x16 ASCII Font Library (Uppercase & Lowercase)
 * ======================================== */

/**
 * @brief 8x16 ASCII Font - Row-Major Format
 * Source: Based on standard PC BIOS 8x16 font
 * Format: Each character is 16 bytes (16 rows × 1 byte per row, 8 pixels per row)
...
static const uint8_t font_8x16[95][16] = {
	...
	/* 0x7C (124) | */
	{0x00, 0x00, 0x18, 0x18, 0x18, 0x18, 0x00, 0x00, 0x00, 0x18, 0x18, 0x18, 0x18, 0x00, 0x00, 0x00},
	/* 0x7D (125) } */
...
/**
 * @brief Draw a single 8x16 character (public API)
 * @param buf Frame buffer
...
void display_draw_char_8x16(uint8_t *buf, char c, int x, int y)
{
	...
	int idx = c - 32;
	const uint8_t *font_data = font_8x16[idx];

	/* Draw character (8x16, row-major) */
...
/**
 * @brief Draw a string using 8x16 font (public API)
 * @param buf Frame buffer
...
{
	int cur_x = x;
	while (*str && cur_x < OLED_WIDTH - 8) {
		display_draw_char_8x16(buf, *str, cur_x, y);
		cur_x += 8;  /* Character width + 1 pixel gap */
		str++;
	}
	return cur_x;
}
...
static void render_pairing_guide_page(uint8_t *buf)
{
	...
	int text_start_x = OLED_WIDTH - PAIRING_TEXT_RIGHT_MARGIN - 8;  /* -8 for character width */
	...
	for (const char *p = line1; *p; p++) {
		display_draw_char_8x16(buf, *p, x, text_y);
		x += 8;  /* Character width (8x16 font), no gap between letters */
	}
	...
	x = text_start_x;
	text_y += 16 + PAIRING_LINE_SPACING;  /* Move to next line */
