...
**Fields:**
- `timestamp`: Unix timestamp when bookmark added
- `offset`: Seconds from session start
- `file`: Current audio file name
- `file_offset`: File index in session
...
[4 bytes offset: uint32 - seconds from start]
[2 bytes file_index: uint16]
[4 bytes file_offset: uint32]
[2 bytes note_length: uint16]
[N bytes note: UTF-8 string]
```

**Total entry size:** 22 + note_length bytes
...
struct __attribute__((packed)) mark_entry {
    ...
    char note[];  // Flexible array member
};
```

### 6.4 Opus Frame Format
