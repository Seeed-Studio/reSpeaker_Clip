 ...
 * BEFORE spi_nor runs. */
static int flash_vdd_preinit(void)
{
...
static int cmd_sys_stop(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* 1. Stop the 1 s battery/OLED refresh work — otherwise it keeps waking
	 * the CPU every second and writing to the bus/display being shut down. */
	shell_print(sh, "  [1/9] Stop 1s battery/OLED refresh work: %d",
		    pmic_display_work_stop());

	/* 2. BLE / network-core radio (wake source) */
	shell_print(sh, "  [2/9] BLE: adv stop + disable");
	ret = bt_le_adv_stop();
	if (ret != 0 && ret != -EALREADY) {
	...
	/* 3. nRF7002 WiFi supply (~70 uA) */
	shell_print(sh, "  [3/9] WiFi: nRF7002 power off");
	sys_wifi_power_off();

	/* 4. USB device (wake source when VBUS present) */
	shell_print(sh, "  [4/9] USB: disable");
	(void)usb_msc_disable();

	/* 5. SD card + LDO2 */
	shell_print(sh, "  [5/9] SD: unmount + SPI4 suspend + LDO2 off");
	sd = sys_sd_shutdown();
	if (sd.ldo_after) {
	...
	/* 6. Mic LDO1 */
	shell_print(sh, "  [6/9] Mic: LDO1 off");
	if (device_is_ready(ldo1)) {
		(void)regulator_disable(ldo1);
	}

	/* 7. OLED (bus suspend + VDD/RESET low to avoid ESD back-power) */
	shell_print(sh, "  [7/9] OLED: bus suspend + VDD off");
	(void)pm_device_action_run(i2c2, PM_DEVICE_ACTION_SUSPEND);
	(void)gpio_pin_configure(gpio1, 9, GPIO_OUTPUT_INACTIVE);  /* RESET */
	(void)gpio_pin_configure(gpio1, 8, GPIO_OUTPUT_INACTIVE);  /* OLED VDD */

	/* 8. BUCK2 PFM (low-power regulator mode) */
	shell_print(sh, "  [8/9] BUCK2: PFM");
	if (device_is_ready(buck2)) {
		(void)regulator_set_mode(buck2, NPM13XX_BUCK_MODE_PFM);
	}

	/* 9. UART console (~570 uA) — suspended LAST so the shell stays up until
	 * the final message is flushed. */
	shell_print(sh, "  [9/9] UART: suspend (console goes dark) -> SYSTEM OFF");
...
static int cmd_hfxo_cap_set(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_print(sh, "HFXO capacitance set to: %.1f pF (CAPVALUE=%u)", (double)pf, capvalue);
	return 0;
}
